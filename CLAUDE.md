...
- **Content & CMS.** All site copy lives in CMS-owned JSON under `content/` and is edited through **Sveltia CMS** at `/admin` (config in `public/admin/config.yml`, regenerated by `node scripts/gen-cms-config.mjs`). `lib/content.ts` is a typed adapter that imports `content/*.json` and re-exports it under the names the components use. **Edit copy in `content/*.json` (or via `/admin`), never inline in sections and never in `lib/content.ts`.** Files: `content/home.json` (homepage, incl. all section headings + Coverage/Continuity), `content/team.json`, `content/advisory.json`, `content/global.json` (nav/footer), `content/services/<slug>.json` (the six service pages). The `/admin` route is served via a rewrite in `next.config.ts`. See `docs/CMS.md` for the editor guide and the "before publishing" checklist (bracketed `[…]` placeholders + open decisions like advisory price and the hidden podcast).
- **Code layout:**
  - `lib/content.ts` — typed re-export layer over `content/*.json` (see Content & CMS above).
