        ...
        <Reveal delay={0.05}>
          <Accordion.Root type="single" collapsible className="mt-10 max-w-[780px]">
            {faqs.map((item, i) => (
              ...
              >
                <Accordion.Header>
                  <Accordion.Trigger className="group flex w-full items-center justify-between gap-4 py-[22px] text-left font-display text-[21px] font-semibold text-ink">
                    {item.q}
                    <Plus
                      className="size-[22px] flex-none text-gold transition-transform duration-300 group-data-[state=open]:rotate-45 motion-reduce:transition-none"
                      strokeWidth={1.6}
                    />
                  </Accordion.Trigger>
                </Accordion.Header>
